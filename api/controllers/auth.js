                ...
                if (checkUserExist(userID)) {
                    ...
                    if (result) {
                        console.log(
                            "user:",
                            userID,
                            "has been found & data successfully updated"
                        );
                        res
                            .status(200)
                            .json({
    ...
    upadtePassword: async (req, res, next) => {
        try {
            if (res.locals.user) {
                ...
                const validationResult = await passwordUpdateSchema.validateAsync(
                    req.body
                );
                ...
                if (result) {
                    ...
                    console.log(
                        "user:",
                        userID,
                        "with given password doesn't exist in database"
                    );
                    res
                        .status(404)
                        .json({
    ...
    updateEmail: async (req, res, next) => {
        try {
            if (res.locals.user) {
                ...
                const validationResult = await emailUpdateSchema.validateAsync(
                    req.body
                );
                ...
                if (result) {
                    ...
                    console.log(
                        "user:",
                        userID,
                        "with given credntials doesn't exist in database"
                    );
                    res.status(404).json({
                        success: false,
